...
def test_extract_json_records(tmp_path):
    ...
    client = GeminiClient(api_key="fake", model="test-model", record_responses=True, record_path=str(tmp_path))
    ...
    # Check contents
    raw = next(p for p in files if p.name.endswith("_raw.txt")).read_text(encoding="utf-8")
    parsed_json = json.loads(next(p for p in files if p.name.endswith("_parsed.json")).read_text(encoding="utf-8"))
